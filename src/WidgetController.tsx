...
export default function WidgetController() {
  ...
  const { count, limit, widgetTheme, drinkEvents, unit, drinkType } = state;

  const std = sessionStdCount(drinkEvents, count, unit, drinkType);
  // 위젯 "+1"(현재 주종 한 잔)이 더해질 때 늘어나는 표준잔. 낙관적 갱신을 앱과 일치시킨다.
  const perDrinkStd = alcoholGrams(1, unit, drinkType) / STD_GRAMS;
